    ...
    $('.orders-final .submit').on('click', function() {
        const formHtml = `
            <form id="ordersSubmitForm">
                ...
                <button type="button" class="confirmOrdersBtn m-1">Confirm</button>
                <button type="button" class="cancelOrdersBtn m-1">Cancel</button>
            </form>
        `;
        showModal(formHtml);
    });

    $('#modalOverlay').on('click', '.cancelOrdersBtn', function() {
        hideModal();
    });
}